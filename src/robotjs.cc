...
NAN_METHOD(mouseClick)
{
	...
	if (info.Length() > 0)
	{
		Nan::Utf8String bstr(info[0]->ToString(Nan::GetCurrentContext()).FromMaybe(v8::Local<v8::String>()));
		const char * const b = *bstr;
...
{
	Nan::Utf8String fstr(value->ToString(Nan::GetCurrentContext()).FromMaybe(v8::Local<v8::String>()));
...
int GetFlagsFromValue(v8::Local<v8::Value> value, MMKeyFlags* flags)
{
	...
	if (value->IsArray())
	{
		...
		{
			v8::Local<v8::Value> v(a->Get(Context::New(v8::Context::GetIsolate()), i));
...
NAN_METHOD(keyTap)
{
	...
	Nan::Utf8String kstr(info[0]->ToString(Nan::GetCurrentContext()).FromMaybe(v8::Local<v8::String>()));
	k = *kstr;
